CREATE OR REPLACE FUNCTION public.can_manage_revelao_event(target_event_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
  ...
  SELECT auth.uid() IS NOT NULL
    AND EXISTS (
      ...
      WHERE e.id = target_event_id
        AND (
          e.owner_id = auth.uid()
          OR lower(coalesce(auth.jwt() ->> 'email', '')) = 'revelao.cam@gmail.com'
        )
    );
$$;
...
DROP POLICY IF EXISTS "Managers can view event media objects" ON storage.objects;
CREATE POLICY "Managers can view event media objects"
ON storage.objects
FOR SELECT
TO authenticated
USING (
...
ALTER TABLE public.events ADD CONSTRAINT events_type_check CHECK (type IN ('demo', 'paid', 'capsule'));