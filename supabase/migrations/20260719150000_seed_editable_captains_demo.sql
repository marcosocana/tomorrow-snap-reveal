-- Persist the public Captains demo so revelao.cam@gmail.com can edit it from the admin.
...
BEGIN
  ...
  IF options_type = 'jsonb' THEN
    EXECUTE $sql$
      ...
      SET question_options = '["En un restaurante","En la playa","En un concierto","En casa de amigos"]'::jsonb
      WHERE id = 'dc000000-0000-4000-8000-000000000002'
    $sql$;
  ELSE
    EXECUTE $sql$
      UPDATE public.captains_event_challenges
      SET question_options = ARRAY['En un restaurante', 'En la playa', 'En un concierto', 'En casa de amigos']::text[]
      WHERE id = 'dc000000-0000-4000-8000-000000000002'
    $sql$;
  END IF;
END;
...
CREATE TRIGGER protect_captains_demo_event
BEFORE UPDATE OR DELETE ON public.captains_events
...
CREATE TRIGGER protect_captains_demo_tables
BEFORE INSERT OR UPDATE OR DELETE ON public.captains_tables
FOR EACH ROW EXECUTE FUNCTION public.protect_captains_demo_from_non_admin_changes();

DROP TRIGGER IF EXISTS protect_captains_demo_challenges ON public.captains_event_challenges;
CREATE TRIGGER protect_captains_demo_challenges
BEFORE INSERT OR UPDATE OR DELETE ON public.captains_event_challenges
FOR EACH ROW EXECUTE FUNCTION public.protect_captains_demo_from_non_admin_changes();

DROP TRIGGER IF EXISTS protect_captains_demo_progress ON public.captains_table_challenges;
CREATE TRIGGER protect_captains_demo_progress
BEFORE INSERT OR UPDATE OR DELETE ON public.captains_table_challenges
FOR EACH ROW EXECUTE FUNCTION public.protect_captains_demo_from_non_admin_changes();

DROP TRIGGER IF EXISTS protect_captains_demo_evidence ON public.captains_evidence;
CREATE TRIGGER protect_captains_demo_evidence
BEFORE INSERT OR UPDATE OR DELETE ON public.captains_evidence
FOR EACH ROW EXECUTE FUNCTION public.protect_captains_demo_from_non_admin_changes();
