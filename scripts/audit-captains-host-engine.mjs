...
import { defaultCaptainsHostConfig, normalizeCaptainsHostConfig } from "../src/lib/captainsHosts.ts";
...
assert.equal(defaultCaptainsHostConfig().frequency, "high");
assert.equal(normalizeCaptainsHostConfig().frequency, "high");
assert.equal(normalizeCaptainsHostConfig({ host_frequency: "low" }).frequency, "low");
assert.equal(normalizeCaptainsHostConfig({ host_frequency: "normal" }).frequency, "normal");
...
for (const frequency of ["low", "normal", "high"]) {
  assert.equal(choose(snapshot({ points: 50 }), snapshot({ points: 40 }), [record("FIRST_CHALLENGE_COMPLETED", 0)], frequency)?.trigger, "POINTS_50");
  assert.equal(choose(snapshot({ points: 100 }), snapshot({ points: 80 }), [record("POINTS_50", 0)], frequency)?.trigger, "POINTS_100");
  assert.equal(choose(snapshot({ points: 50 }), snapshot({ points: 40 }), [record("POINTS_50", 0)], frequency), null);
...
assert.equal(choose(snapshot({ points: 50 }), snapshot({ points: 40 }), [record("FIRST_CHALLENGE_COMPLETED", 1)])?.trigger, "POINTS_50");
...
console.log("PASS: high frequency defaults, host triggers, thresholds, transitions, deduplication, no pauses at any frequency, priority, disabled state and safe templates.");
