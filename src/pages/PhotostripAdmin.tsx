...
export const PhotostripAdminForm = ({ edit = false }: { edit?: boolean }) => {
  ...
  return <div className="min-h-screen bg-muted/20"><AdminHeader title={edit ? "Editar evento" : "Nuevo evento"} />
    <main className="mx-auto max-w-3xl p-4 py-8"><form onSubmit={save}><Card className="space-y-6 p-5 md:p-7">
      ...
      <label className="block space-y-2 text-sm font-medium">URL pública<div className="flex items-center rounded-md border bg-background"><span className="pl-3 text-xs text-muted-foreground">/photostrip/</span><Input required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" className="border-0" value={form.slug} onChange={(e) => { setSlugTouched(true); update("slug", slugify(e.target.value)); }} /></div></label>
      ...
      <div className="flex justify-end gap-3"><Button type="button" variant="outline" onClick={() => navigate(-1)}>Cancelar</Button><Button disabled={saving}>{saving ? "Guardando…" : "Guardar Photostrip"}</Button></div>
    </Card></form></main></div>;
};
...
export const PhotostripAdminDetail = () => {
  ...
  return <div className="min-h-screen bg-muted/20"><AdminHeader title={event.name} /><main className="mx-auto max-w-6xl space-y-6 p-4 py-8">
