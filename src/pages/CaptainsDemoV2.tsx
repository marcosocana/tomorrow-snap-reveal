...
export default function CaptainsDemoV2({ eventSlug: requestedEventSlug }: { eventSlug?: string } = {}) {
  ...
  return <div className={`cv2 cv2-mobile ${isDemoEvent ? "is-demo-event" : ""} ${cameraActive ? "is-camera-active" : ""}`} style={{ "--team": team?.color ?? "#f06a5f" } as CSSProperties}>
    ...
    <main ref={mainRef} className={`cv2-mobile-main ${showWelcome || (!started && !game.eventEnded) ? "is-welcome" : ""} ${activeMission ? "is-mission" : ""} ${file ? "has-media-preview" : ""}`}>
      ...
      {(started || game.eventEnded) && game.data && joined && activeMission ? <section className="cv2-mission-screen" aria-labelledby="cv2-mission-title">
        ...
        {view === "quests" && <section className="cv2-mobile-quests" aria-labelledby="cv2-quests-title">
          ...
          {!experienceFinished && <div className="cv2-mission-path" aria-label="Próximos retos">{missions.map((item, index) => ({ item, index })).filter(({ index }) => index > completed).slice(0, 5).map(({ item, index }) => {
    ...
    <Dialog open={Boolean(hosts.active && hosts.message && mission === null)} onOpenChange={() => undefined}><DialogContent className="cv2-dialog cv2-host-dialog" onEscapeKeyDown={event => event.preventDefault()} onPointerDownOutside={event => event.preventDefault()}>
      {hosts.message && <HostIntervention config={hosts.config} message={hosts.message} onContinue={() => void hosts.dismiss().then(() => navigate("quests"))} />}
