...
import { ArrowLeft, ArrowRight, Camera, Check, CircleDashed, Crown, Film, Flag, HelpCircle, LockKeyhole, RotateCcw, Trophy, Clock3, Loader2, XCircle } from "lucide-react";
...
export default function CaptainsDemoV2({ eventSlug: requestedEventSlug }: { eventSlug?: string } = {}) {
  ...
  const [endedIdentityChosen, setEndedIdentityChosen] = useState(false);
  const selected = game.eventEnded && !endedIdentityChosen ? choice : game.selected ?? choice;
  const joined = game.selected !== null && (!game.eventEnded || endedIdentityChosen);
  ...
  const experienceFinished = finished || game.eventEnded;
  ...
  }, [started, joined, view, mission, celebrating, hasMedia, cameraActive, completed, eventSlug]);
  ...
  useEffect(() => {
    ...
    if (!existing) document.head.appendChild(viewport);
    const restoration = history.scrollRestoration;
    history.scrollRestoration = "manual";
    document.documentElement.classList.add("captains-game-open");
    ...
    return () => {
      if (!existing) viewport.remove();
      else if (original === null) viewport.removeAttribute("content");
      else viewport.content = original;
      history.scrollRestoration = restoration;
      document.documentElement.classList.remove("captains-game-open");
  ...
  const join = async () => {
    ...
    if (await game.join(selected, name)) { setEndedIdentityChosen(true); window.scrollTo({ top: 0, behavior: "instant" }); }
  };
  ...
  const navigate = (next: View) => { setView(next); mainRef.current?.scrollTo({ top: 0, behavior: "instant" }); window.scrollTo({ top: 0, behavior: "instant" }); };
  const openResultMedia = async (item: (typeof memories)[number], title: string) => {
    const type = item.evidence_type === "video" ? "video" : "photo";
    ...
    try {
      ...
      setPreviewMedia(current => current?.title === title ? { ...current, error: `No se ha podido cargar ${type === "video" ? "el vídeo" : "la foto"}.` } : current);
    }
  };
  ...
  return <div className="cv2 cv2-mobile" style={{ "--team": team?.color ?? "#f06a5f" } as CSSProperties}>
    ...
    <main ref={mainRef} className={`cv2-mobile-main ${showWelcome || (!started && !game.eventEnded) ? "is-welcome" : ""} ${activeMission ? "is-mission" : ""} ${file ? "has-media-preview" : ""}`}>
      ...
      {(started || game.eventEnded) && game.data && joined && activeMission ? <section className="cv2-mission-screen" aria-labelledby="cv2-mission-title">
        ...
        {celebrating ? <><div className="cv2-celebration-points">+{result.pointsAwarded}<span>puntos para vuestra mesa</span></div><button className="cv2-primary cv2-centered-action cv2-mission-submit" onClick={closeMission}>{finished ? "Ver nuestra victoria" : "Descubrir siguiente reto"}</button></> : <>
          ...
          {isQuestion && submitButton}
        </>}
      </section> : !game.data ? <div className="cv2-loading" role="status">{game.loading && <Loader2 className="animate-spin" />}<h1>{game.loading ? "Preparando vuestra mesa…" : "Estamos preparando la partida"}</h1></div> : showWelcome || (!started && !game.eventEnded) ? <section className="cv2-welcome" aria-labelledby="cv2-welcome-title">
        <div className="cv2-welcome-art" aria-hidden="true"><img src={captainArmbands} alt="" /></div>
        ...
        <div className="cv2-join-bar"><button className="cv2-primary cv2-centered-action" onClick={() => { setShowWelcome(false); setStarted(true); window.scrollTo({ top: 0, behavior: "instant" }); }}>Empezar</button></div>
      </section> : !joined ? <>
        ...
        {view === "quests" && <section className="cv2-mobile-quests" aria-labelledby="cv2-quests-title">
          ...
          {experienceFinished ? null : current ? <article key={current.id} className="cv2-active-quest cv2-quest-enter" style={{ animationPlayState: hosts.active ? "paused" : "running" }}>
            <div className="cv2-card-top"><span className="cv2-eyebrow"><span className="cv2-status-dot" /> Reto {String(completed + 1).padStart(2, "0")}</span><span className="cv2-points">{current.points} puntos</span></div><div className="cv2-quest-object"><current.icon size={52} strokeWidth={1.3} /><span>✦</span></div><span className="cv2-mission-type">{current.type}</span><h2>{current.title}</h2><p>{current.description}</p><button className="cv2-primary cv2-centered-action" disabled={game.busy} onClick={openMission}>{game.busy ? "Abriendo…" : game.currentRow?.status === "in_progress" ? "Continuar reto" : "Aceptar reto"}</button><button className="cv2-secondary cv2-reject-button" disabled={game.busy} onClick={() => setRejecting(true)}>Rechazar reto</button>
          ...
          {!experienceFinished && <div className="cv2-mission-path" aria-label="Próximos retos">{missions.map((item, index) => ({ item, index })).filter(({ index }) => index > completed).slice(0, 5).map(({ item, index }, futureIndex) => {
            return <div key={item.id} className={`cv2-locked-quest ${futureIndex >= 3 ? `is-depth-fade fade-${futureIndex - 2}` : ""}`} aria-label={`Reto ${index + 1} bloqueado. Completa el reto anterior para descubrirlo.`}><span className="cv2-path-number">0{index + 1}</span><div className="cv2-locked-content" aria-hidden="true"><item.icon size={26} /><div><strong>{item.title}</strong><small>{item.type}</small></div></div><span className="cv2-lock-label"><LockKeyhole size={16} /> Reto sorpresa<span>Completa el anterior</span></span></div>;
          })}</div>}
        </section>}
        {view === "ranking" && <section className="cv2-mobile-ranking"><div className="cv2-view-intro"><h2>Camino a la gloria.</h2><p>Vais en el puesto #{position}.</p></div><div className="cv2-podium">{[1, 0, 2].map(place => {
          const item = ranking[place];
          return item && <div key={item.id} className="cv2-podium-team" style={tableCaptainStyle(item.index, item.captain_sprite, item.captain_sprite_config)}>{place === 0 && <Crown className="cv2-winner-crown" size={27} />}<Captain index={item.index} photoUrl={item.captain_photo_url} sprite={item.captain_sprite} config={item.captain_sprite_config} /><span className={`cv2-podium-step place-${place}`}>{place === 0 && <Trophy size={29} />}<b>{place + 1}</b><strong>{item.table_name}</strong><small>{item.points} puntos</small></span></div>;
        })}</div><div className="cv2-mobile-ranks">{ranking.map((item, place) => <div key={item.id} className={item.index === selected ? "is-you" : ""}><span>0{place + 1}</span><strong>{item.table_name}<small>{item.index === selected ? "Vuestro equipo" : item.name}</small></strong><b>{item.points} <small>pts</small></b>{place === 0 && <Crown size={18} />}</div>)}</div><p className="cv2-ranking-rule"><Clock3 size={14} /> En caso de empate, gana la mesa que haya completado todos los retos en menos tiempo.</p><p className="cv2-demo-note">Clasificación compartida · Se actualiza automáticamente</p></section>}
        {view === "memories" && experienceFinished && <section className="cv2-mobile-memories"><div className="cv2-view-intro"><span className="cv2-eyebrow">RESULTADOS</span><h2>Retos</h2><p>Revisa las fotos, vídeos y respuestas de cada mesa.</p></div><label className="cv2-gallery-filter">Mesa<select value={galleryTable} onChange={event => setGalleryTable(event.target.value)}><option value="mine">Mi mesa</option>{liveTeams.filter(item => item.id !== team?.id).map(item => <option key={item.id} value={item.id}>{item.table_name}</option>)}</select></label>
          ...
          <div className="cv2-memory-grid">{resultRows.map((row, index) => {
            ...
            const media = item ? { thumbnailUrl: item.thumbnailUrl, type: item.evidence_type === "video" ? "video" as const : "photo" as const, title: task?.title ?? "Reto", item } : null;
            const successful = row.status === "completed";
            const notPerformed = ["pending", "ready", "in_progress"].includes(row.status);
            const status = successful ? `+${row.points_awarded} puntos` : row.status === "failed" ? "0 puntos" : row.status === "time_expired" ? "Tiempo agotado" : "Aún no realizado";
            return <article className="cv2-memory cv2-result-card" key={row.id}>
              {media ? <button className="cv2-memory-media" onClick={() => void openResultMedia(media.item, media.title)}>{media.thumbnailUrl ? <img src={media.thumbnailUrl} alt={`${media.type === "video" ? "Primer fotograma" : "Miniatura"} de ${media.title}`} loading="lazy" /> : <span className="cv2-video-placeholder">{media.type === "video" ? <Film size={34} /> : <Camera size={34} />}<small>Abrir {media.type === "video" ? "vídeo" : "foto"}</small></span>}</button> : <span className={`cv2-result-placeholder ${successful ? "is-success" : ""} ${notPerformed ? "is-pending" : ""}`}>{notPerformed ? <><CircleDashed size={30} /><small>Aún no realizado</small></> : task?.evidence_type === "question" ? <HelpCircle size={35} /> : successful ? <Check size={35} /> : <CircleDashed size={30} />}</span>}
              <span className="cv2-memory-caption"><small>RETO 0{index + 1} · {status}</small><strong>{notPerformed ? task?.title : task?.evidence_type === "question" ? task.description || task.title : task?.title}</strong>{task?.evidence_type === "question" && row.question_answer && <span className="cv2-result-answer">Opción marcada: {row.question_answer}</span>}{media && <button onClick={() => void openResultMedia(media.item, media.title)}>Abrir {media.type === "video" ? "vídeo" : "foto"} <ArrowRight size={13} /></button>}</span>
            </article>;
          })}</div>
        </section>}
    ...
    <Dialog open={rejecting} onOpenChange={open => { if (!game.busy) setRejecting(open); }}><DialogContent className="cv2-dialog cv2-confirm-dialog">
      ...
      <button className="cv2-secondary cv2-centered-action" disabled={game.busy} onClick={() => setRejecting(false)}>Volver al reto</button>
      <button className="cv2-primary cv2-centered-action" disabled={game.busy} onClick={rejectMission}>{game.busy ? "Rechazando…" : "Sí, rechazar reto"}{game.busy && <Loader2 size={18} className="animate-spin" />}</button>
    </DialogContent></Dialog>
    <Dialog open={previewMedia !== null} onOpenChange={open => { if (!open) setPreviewMedia(null); }}><DialogContent className="cv2-dialog cv2-media-dialog">
      ...
      {previewMedia?.error ? <p className="cv2-error" role="alert">{previewMedia.error}</p> : previewMedia && !previewMedia.url ? <div className="cv2-media-loading" role="status"><Loader2 className="animate-spin" /><span>Cargando {previewMedia.type === "video" ? "vídeo" : "foto"}…</span></div> : previewMedia?.type === "video" ? <video src={previewMedia.url} controls autoPlay playsInline preload="metadata" /> : previewMedia && <img src={previewMedia.url} alt={previewMedia.title} />}
    </DialogContent></Dialog>
  </div>;
}
