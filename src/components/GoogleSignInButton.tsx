...
const GoogleSignInButton = ({ onClick, loading = false, disabled = false, label = "Continuar con Google" }: GoogleSignInButtonProps) => (
  <Button
    type="button"
    variant="outline"
    className="h-12 w-full rounded-full bg-background text-base font-semibold"
    onClick={onClick}
    disabled={disabled || loading}
  >
    <GoogleIcon />
    <span className="ml-3">{loading ? "Conectando con Google..." : label}</span>
  </Button>
);
