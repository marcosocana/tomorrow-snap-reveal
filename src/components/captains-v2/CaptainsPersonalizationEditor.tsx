...
function CharacterEditor({ number, config, partnerName, onChange }: { number: 1 | 2; config: CaptainsHostCharacterConfig; partnerName: string; onChange: (next: CaptainsHostCharacterConfig) => void }) {
  ...
  return <div className="grid gap-5 rounded-2xl border border-border bg-background p-4 lg:grid-cols-[180px_1fr]">
    ...
    <div className="space-y-4">
      ...
      {choice("Tono de piel", [{value:"light",label:"Claro"},{value:"medium",label:"Medio"},{value:"tan",label:"Moreno"},{value:"deep",label:"Oscuro"}], config.skin_tone, value => patch({ skin_tone: value }))}
      {choice("Pelo", [{value:"short",label:"Corto"},{value:"wave",label:"Ondulado"},{value:"long",label:"Largo"},{value:"curly",label:"Rizado"},{value:"bald",label:"Sin pelo"}], config.hair_style, value => patch({ hair_style: value }))}
      {choice("Color de pelo", [{value:"blonde",label:"Rubio"},{value:"brown",label:"Castaño"},{value:"dark",label:"Oscuro"},{value:"red",label:"Pelirrojo"},{value:"gray",label:"Gris"}], config.hair_color, value => patch({ hair_color: value }))}
      {choice("Barba", [{value:"none",label:"Sin barba"},{value:"stubble",label:"Sombra"},{value:"beard",label:"Barba"}], config.facial_hair, value => patch({ facial_hair: value }))}
      {choice("Gafas", [{value:"none",label:"Sin gafas"},{value:"round",label:"Redondas"},{value:"square",label:"Cuadradas"}], config.glasses, value => patch({ glasses: value }))}
      {choice("Outfit", [{value:"classic_suit",label:"Traje clásico"},{value:"informal_suit",label:"Traje informal"},{value:"classic_dress",label:"Vestido clásico"},{value:"modern_dress",label:"Vestido moderno"},{value:"party",label:"Fiesta"},{value:"casual",label:"Casual"},{value:"custom",label:"Personalizado"}], config.outfit, value => patch({ outfit: value }))}
      <div className="grid grid-cols-2 gap-3"><label className="space-y-1"><span className="text-xs text-muted-foreground">Color principal</span><Input type="color" value={config.primary_color} onChange={event => patch({ primary_color: event.target.value })}/></label><label className="space-y-1"><span className="text-xs text-muted-foreground">Color secundario</span><Input type="color" value={config.secondary_color} onChange={event => patch({ secondary_color: event.target.value })}/></label></div>
      {choice("Accesorio", [{value:"none",label:"Ninguno"},{value:"bowtie",label:"Pajarita"},{value:"tie",label:"Corbata"},{value:"bouquet",label:"Ramo"},{value:"glass",label:"Copa"}], config.accessory, value => patch({ accessory: value }))}
      {choice("Cabeza", [{value:"none",label:"Ninguno"},{value:"hat",label:"Sombrero"},{value:"crown",label:"Corona"}], config.head_accessory, value => patch({ head_accessory: value }))}
    </div>
  </div>;
}
...
export default function CaptainsPersonalizationEditor({ value, onChange }: { value: CaptainsHostConfig; onChange: (next: CaptainsHostConfig) => void }) {
  ...
  return <section className="space-y-4 rounded-2xl border border-[#f06a5f]/25 bg-[#f06a5f]/[.035] p-4 md:col-span-2" aria-labelledby="captains-personalization-title">
    ...
    <Accordion type="multiple" defaultValue={["couple"]} className="space-y-2">
      ...
      <AccordionItem value="characters" className="rounded-xl border bg-background px-4"><AccordionTrigger>3. Los personajes</AccordionTrigger><AccordionContent className="space-y-4"><p className="text-xs text-muted-foreground">Así aparecerán durante el juego. Cada personaje se compone con SVG ligero y se actualiza al instante.</p><CharacterEditor number={1} config={value.character_1} partnerName={wedding.partner_1_name} onChange={character_1 => onChange({ ...value, character_1 })}/><CharacterEditor number={2} config={value.character_2} partnerName={wedding.partner_2_name} onChange={character_2 => onChange({ ...value, character_2 })}/></AccordionContent></AccordionItem>
      <AccordionItem value="personality" className="rounded-xl border bg-background px-4"><AccordionTrigger>4. Personalidad del juego</AccordionTrigger><AccordionContent className="space-y-5">{choice("Tono de los mensajes", toneOptions, value.tone, tone => onChange({ ...value, tone }))}<fieldset className="space-y-2"><legend className="text-xs font-medium text-muted-foreground">Frecuencia de apariciones</legend><div className="grid gap-2 sm:grid-cols-3">{frequencyOptions.map(item => <button type="button" key={item.value} aria-pressed={value.frequency === item.value} onClick={() => onChange({ ...value, frequency: item.value })} className={`rounded-xl border p-3 text-left ${value.frequency === item.value ? "border-[#f06a5f] bg-[#f06a5f]/10" : "bg-background"}`}><strong className="block text-sm">{item.label}</strong><span className="text-xs text-muted-foreground">{item.detail}</span></button>)}</div></fieldset></AccordionContent></AccordionItem>
