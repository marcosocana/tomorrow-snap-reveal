...
function CharacterEditor({ number, config, name, nickname, onWeddingChange, onChange }: { number: 1 | 2; config: CaptainsHostCharacterConfig; name: string; nickname: string; onWeddingChange: (patch: Partial<CaptainsWeddingContext>) => void; onChange: (next: CaptainsHostCharacterConfig) => void }) {
  ...
  return <section className="grid gap-5 rounded-2xl border border-border bg-card p-5 shadow-sm lg:h-[70vh] lg:min-h-[620px] lg:grid-cols-[220px_minmax(0,1fr)] lg:overflow-hidden">
    <div className="mx-auto w-48 self-start lg:sticky lg:top-0"><HostCharacterAvatar config={config} /><p className="mt-2 text-center text-sm font-semibold">{nickname || name || `Novio ${number}`}</p><p className="mt-1 text-center text-xs text-muted-foreground">La previsualización se actualiza al editar.</p></div>
    <div className="min-w-0 space-y-4 lg:overflow-y-auto lg:pr-3">
...
export default function CaptainsPersonalizationEditor({ value, onChange }: { value: CaptainsHostConfig; onChange: (next: CaptainsHostConfig) => void }) {
  ...
  return <section className="space-y-5 md:col-span-2" aria-labelledby="captains-personalization-title">
    ...
    <section className="rounded-2xl border border-border bg-card p-5 shadow-sm"><h3 className="font-semibold">Datos generales de la boda</h3><p className="mt-1 text-xs text-muted-foreground">Solo pedimos la información que se utiliza en los comentarios de los novios.</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="space-y-1"><span className="text-xs font-medium text-muted-foreground">Años juntos</span><Input type="number" min="0" value={wedding.years_together ?? ""} onChange={event => patchWedding({ years_together: event.target.value === "" ? null : Number(event.target.value) })}/></label><label className="space-y-1"><span className="text-xs font-medium text-muted-foreground">Lugar de celebración</span><Input value={wedding.venue_name} placeholder="Finca, restaurante, hotel..." onChange={event => patchWedding({ venue_name: event.target.value })}/></label></div></section>
